...
import os, re, difflib, io, csv, tempfile
...
app = Flask(__name__)
# Vercel's filesystem is read-only except /tmp, so uploads must go there.
# tempfile.gettempdir() resolves correctly both locally and on Vercel.
UPLOAD_DIR = Path(tempfile.gettempdir()) / "policylens_uploads"
